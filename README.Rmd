...
3. `pasttense_rule` = should -d and -ed removed (is not applied when `stemmed_rule` is applied); default is `TRUE`
4. `a_the_rule` = should a and the be considered the the same; default is `TRUE`
5. `plural_rule` = should we remove -s and -es at the end of words (plurals); default is `TRUE`
...
9. `double_letter_rule` = should double letters within words be considered the same as if there was only one? E.g., "attack" is a match with "atack"; default = TRUE
...
[![]("Data/Figures/online_autoscore_snapshot.png")](https://tysonstanley.shinyapps.io/autoscore/)
